"""Anomaly Detection for MDQ Data

This script is designed to detect anomalies in the MDQ dataset using various anomaly detection models 
...
Usage:
- Ensure the required dataset (`preprocessed_data.csv`) is located in the `anomalies/MDQ/csv/` directory.
...
df = pd.read_csv(os.path.join(current_dir, 'MDQ', 'csv', 'preprocessed_data.csv'))
...
def predict_model(model, df, name):
    ...
    df_answers.to_csv(os.path.join(current_dir, 'MDQ', 'csv', f'{name}.csv'))
