"""Anomaly Detection for PAQ Data

This script is designed to detect anomalies in the PAQ dataset using various anomaly detection models 
...
Usage:
- Ensure the required dataset (`preprocessed_data.csv`) is located in the `PAQ/csv/` directory.
...
df = pd.read_csv(os.path.join(current_dir, 'PAQ', 'csv', 'preprocessing_pyod.csv'))
...
def predict_model(model, name):
    ...
    df_answers.to_csv(os.path.join(current_dir, 'PAQ', 'csv', f'{name}.csv'))
