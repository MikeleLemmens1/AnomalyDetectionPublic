"""Anomaly Detection for BAQ Data

This script is designed to detect anomalies in the BAQ dataset using various anomaly detection models 
...
Usage:
- Ensure the required dataset (`preprocessed_data.csv`) is located in the `anomalies/BAQ/csv/` directory.
...
df_norm = pd.read_csv(os.path.join(current_dir, 'BAQ', 'csv', 'preprocessed_data_norm.csv'))
df_ips = pd.read_csv(os.path.join(current_dir, 'BAQ', 'csv', 'preprocessed_data_ips.csv'))

# preprocessing
df.set_index(['TestKey'])
...
def predict_model(model, df, name):
    ...
    df_answers.to_csv(os.path.join(current_dir, 'BAQ', 'csv', f'{name}.csv'))
