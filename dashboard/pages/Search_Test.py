"""
This is the search test page. On this page you can search a canidate by testId.
...
# Load data
with st.spinner('We are getting your test data!'):
...
if test_key is None:
    test_key = search_bar.text_input(
        'Search on test key', key="klein_scherm")
else:
    test_key = test_key

with col[1]:
    back_button = st.button("Search new test", key='back')

if back_button and 'test_key' in st.session_state:
    # Reset the session state for candidate_key
    st.session_state.pop('test_key', None)
    st.rerun()

if test_key and not back_button:

    search_bar.empty()
    title_placeholder.empty()

    with col[0]:
        st.title(f'Test Key: {test_key}')

    try:
        selected_test = df[(df.TestKey == int(test_key))]

        fca_test = selected_test[selected_test.Test == 'FCA']
        sjt_test = selected_test[selected_test.Test == 'SJT']
        ...
        data = {
            ...
            "Waarde": [
                [str(selected_test['Test'].values[0])][0],
                [str(selected_test['Date'].values[0])][0],
                [str(selected_test['CandidateKey'].values[0])][0],
                [str(selected_test['Organisation'].values[0])][0],
                [str(selected_test['ChosenLanguage'].values[0])][0],
                [str(selected_test['AnomalyScore'].values[0])][0],
            ]
        }
        ...
        view_df = selected_test[['SameAnswer', "TooSlow", "TooFast", "DBSCAN",
        ...
        if selected_test.TooSlow is not None and not pd.isna(selected_test.TooSlow.iloc[0]) and int(selected_test.TooSlow.iloc[0]) == 1:
            expl = """
            he time the candidate spent on completing the entire test is used to first calucate the average and the standard deviation (std). Afterwards every test is checked and 
        ...
        if selected_test.TooFast is not None and not pd.isna(selected_test.TooFast.iloc[0]) and int(selected_test.TooFast.iloc[0]) == 1:
            expl = """
            he time the candidate spent on completing the entire test is used to first calucate the average and the standard deviation (std). Afterwards every test is checked and 
            tests that took more than the average plus the std are considered significantly slower, while tests that took less than the average minus the std are considered 
            significantly faster. 
        ...
        if selected_test.SameAnswer is not None and not pd.isna(selected_test.SameAnswer.iloc[0]) and int(selected_test.SameAnswer.iloc[0]) == 1:
            expl = """
            ...
            """
            render_detection_message(
                "SameAnswer", "uitleguitleguitleguitleguitleguitleg")

        if selected_test.DBSCAN is not None and not pd.isna(selected_test.DBSCAN.iloc[0]) and int(selected_test.DBSCAN.iloc[0]) == 1:
            expl = """
        ...
        if selected_test.Neighbours is not None and not pd.isna(selected_test.Neighbours.iloc[0]) and int(selected_test.Neighbours.iloc[0]) == 1:
            expl = """
        ...
        if selected_test.PyodEnsemble is not None and not pd.isna(selected_test.PyodEnsemble.iloc[0]) and int(selected_test.PyodEnsemble.iloc[0]) == 1:
            expl = """
        ...
        if selected_test.Copod is not None and not pd.isna(selected_test.Copod.iloc[0]) and int(selected_test.Copod.iloc[0]) == 1:
            expl = """
        ...
        if selected_test.Ecod is not None and not pd.isna(selected_test.Ecod.iloc[0]) and int(selected_test.Ecod.iloc[0]) == 1:
            expl = """
        ...
        if selected_test.iForest is not None and not pd.isna(selected_test.iForest.iloc[0]) and int(selected_test.iForest.iloc[0]) == 1:
            expl = """
        ...
        if selected_test.Lof is not None and not pd.isna(selected_test.Lof.iloc[0]) and int(selected_test.Lof.iloc[0]) == 1:
            expl = """
        ...
        if selected_test.Abod is not None and not pd.isna(selected_test.Abod.iloc[0]) and int(selected_test.Abod.iloc[0]) == 1:
            expl = """
        ...
        if [str(selected_test['Test'].values[0])][0] == 'FCA':
            df_question = df_question_fca[(
                df_question_fca.TestKey == int(selected_test.TestKey))]
        ...
        if button_llm.button("Generate overview"):
            ...
            dict = selected_test.to_dict(orient='records')[0]
        ...
        # Genereer de PDF
        pdf_bytes = exp.export_markdown_to_pdf(selected_test)

        # Bied de PDF aan als downloadbaar bestand
        st.download_button(
            label="Download PDF",
            data=pdf_bytes,
            file_name=f"test_{test_key}.pdf",
            mime="application/pdf"
        )

    except KeyError as k:
        st.error(
            f"Error generating PDF. Please ensure wkhtmltopdf is installed correctly.", icon="🚨")

    except Exception as e:
        # Toon een generieke foutmelding
        st.error(
            "Invalid test ID. Please check your input and try again.", icon="🚨")
